...
from discord_tip_bot import models, store
from discord_tip_bot.config import config

COIN_DIGITS = 1000000000000
COIN_REPR = 'COIN_NAME'
...
async def balance(context: commands.Context):
    ...
    await bot.send_message(
        ...
        f'Available: {wallet.actual_balance / COIN_DIGITS:.12f} '
        f'{COIN_REPR}\n'
        f'Pending: {wallet.locked_balance / COIN_DIGITS:.12f} '
        f'{COIN_REPR}\n')
...
async def withdraw(context: commands.Context, amount: float):
    ...
    await bot.send_message(
        context.message.author,
        f'You have withdrawn {real_amount / COIN_DIGITS:.12f} '
        f'{COIN_REPR}.\n'
...
async def tip(context: commands.Context, member: discord.Member,
    ...
    await bot.reply(f'Tip of {real_amount / COIN_DIGITS:.12f} '
                    f'{COIN_REPR} '
